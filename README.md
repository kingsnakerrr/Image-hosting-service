...
1. 一台 Ubuntu / Debian VPS，开放 TCP 80、443 端口。
...
```text
前端地址：https://img.example.com/
后端地址：https://img.example.com/admin
后台账号：admin
后台随机密码：自动生成的随机密码
```
...
1. 打开后端地址 `https://img.example.com/admin`。
...
5. 前端用户打开 `https://img.example.com/`，使用后台创建的账号密码登录上传。
...
```text
https://img.example.com/i/<192-bit 随机令牌>/example.jpg
```
...
- **网页打不开**：检查域名 A 记录、VPS 防火墙和云厂商安全组的 80/443 端口。
