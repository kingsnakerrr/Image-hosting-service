...
async function requireAdmin(request: FastifyRequest, reply: any) {
  ...
  if (!['GET', 'HEAD', 'OPTIONS'].includes(request.method)) {
    ...
    if (isConfigured() && origin && appUrl && origin !== appUrl) return reply.code(403).send({ error: '请求来源不可信，请从配置的图床域名访问后台' });
  }
}
...
app.post('/api/setup', { preHandler: requireAdmin, config: { rateLimit: { max: 10, timeWindow: '15 minutes' } } }, async (request, reply) => {
  ...
  const body = request.body as { appUrl?: string; sharedDriveId?: string; folderId?: string; serviceAccount?: unknown } | null;
  ...
  let appUrl: string;
  try { appUrl = cleanAppUrl(body.appUrl || ''); } catch (error: any) { return reply.code(400).send({ error: error.message }); }
  try {
